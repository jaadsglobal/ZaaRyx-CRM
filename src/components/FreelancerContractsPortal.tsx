...
export const FreelancerContractsPortal: React.FC = () => {
  ...
  const loadContracts = async (background = false) => {
    ...
    try {
      ...
      console.error('Error loading freelancer contracts:', error);
  ...
  const loadContractEvents = async (contractId: number) => {
    ...
    try {
      ...
      console.error('Error loading freelancer contract events:', error);
      setEvents([]);
  ...
  const scrollToContractList = () => {
    document.getElementById('freelancer-contracts-list-section')?.scrollIntoView({
      behavior: 'smooth',
      block: 'start',
    });
  };
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/50">
            Consulta los acuerdos que la agencia ha compartido contigo, su estado y el historial de cambios.
          </p>
        </div>
      ...
      <CollapsibleSection
        ...
        description="KPIs rápidos sobre contratos visibles, pendientes y firmados."
        icon={<FileText className="w-5 h-5" />}
        summary={`${contracts.length} visibles`}
        storageKey="freelancer-contracts-summary"
      >
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          {[
            ...
            },
            {
              ...
              hint: 'Filtra contratos por revisar',
            },
            {
              label: 'Firmados',
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[0.92fr_1.08fr] gap-6">
        <CollapsibleSection
          title="Tus contratos"
          description="Selecciona un acuerdo para ver el detalle completo."
          icon={<FileText className="w-5 h-5" />}
          summary={`${filteredContracts.length} visibles`}
          storageKey="freelancer-contracts-list"
          actions={
            contractStatusFilter !== 'all' ? (
              <button
                type="button"
        ...
        >
          <div className="space-y-3 max-h-[720px] overflow-y-auto" id="freelancer-contracts-list-section">
            {loading
              ? Array.from({ length: 3 }).map((_, index) => (
                  <div
                    key={`freelancer-contract-skeleton-${index}`}
              ...
              : filteredContracts.length === 0
                ? (
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-white/45">
                      La agencia todavía no ha compartido contratos contigo.
                    </div>
                  )
        ...
        <CollapsibleSection
          ...
          description="Resumen, documento y trazabilidad del acuerdo seleccionado."
          icon={<CheckCircle2 className="w-5 h-5" />}
          summary={selectedContract?.contract_number || 'Sin selección'}
          storageKey="freelancer-contracts-detail"
        >
          {selectedContract ? (
            <>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-2">
                  ...
                  {selectedContract.line_items.length > 0 ? (
                    selectedContract.line_items.map((lineItem) => (
                      <div
                        key={lineItem.id}
                        className="flex items-center justify-between gap-3 text-white/55"
                      >
              ...
              <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                <p className="font-semibold mb-4">Documento compartido</p>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-4">
                ...
                <div className="space-y-3 max-h-[260px] overflow-y-auto">
                  {eventsLoading ? (
                    Array.from({ length: 3 }).map((_, index) => (
                      <div
                        key={`freelancer-contract-event-${index}`}
